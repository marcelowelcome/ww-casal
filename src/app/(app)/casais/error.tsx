...
export default function Error({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => {
    console.error('[casais] route error', error);
  }, [error]);

  return (
    <div className="mx-auto max-w-2xl px-6 py-16 text-center sm:px-8">
      <h1 className="mb-3 font-serif text-h2 text-cocoa-900">
        Não foi possível carregar os <em>casais</em>
      </h1>
      <p className="mb-8 text-body text-cocoa-700">
        Tente novamente em alguns instantes. Se o problema persistir, fale com o time técnico.
      </p>
